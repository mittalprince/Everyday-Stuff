...
Passport is a authentication middleware for Node.js applications. It's extremely flexible and modular, that can be fully customised and works great with express web applications. It is flexible in the sense that it allows for different authentication strategies like Local Strategy(username and password), Facebook, Twitter and more.
...
Once all the prerequisites software is set up, we can start our project. First create the folder for app and write the following command for node app.
...
Here we are using mLab to store data, so we are assuming that you have already an account on mLab. Create a new Mongodb Deployment of plan type SANDBOX. After creating the database add the db User.
...
Now we will congigure out authentication strategy using passport-local. So install it using command:
...
In above code we check if user is login or not.If not then he has option of login and register otherwise Dashboard and logout.
...
We have only added the necessary modules for this app to work - nothing more, nothing less. For a production app you must need to add other middleware  and also we have saved unhashed passwords. So you may use bcryptjs for salting and hashing password. 
